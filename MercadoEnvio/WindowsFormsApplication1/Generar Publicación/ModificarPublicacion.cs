...
namespace MercadoEnvios.Generar_Publicación
{
    public partial class frmModificarPublicacion : Form
    {
        ...
        public void getData()
        {
            string sacarDatosPublicacion = "SELECT id, descripcion, precio, stock, (SELECT nombre FROM ADIOS_TERCER_ANIO.Visibilidad WHERE idVisibilidad = id) AS Visibilidad, tipo, (SELECT descripcionCorta FROM ADIOS_TERCER_ANIO.Rubro WHERE idRubro = id) AS Rubro, iif(tienePreguntas = 0, 'SI', 'NO') AS Acepta_Preguntas, iif(idEnvio IS NULL, 'NO', 'SI') AS Permite_Envios, (SELECT nombre FROM ADIOS_TERCER_ANIO.Estado WHERE idEstado = id) AS Estado, fechaFin AS Fecha_Finalización FROM ADIOS_TERCER_ANIO.Publicacion WHERE @idPublic = idPublicador";
